...
router.get("/", async (_req: Request, res: Response) => {
  try {
    ...
    res.status(500).json({ error: "Failed to aggregate training needs", message: err.message });
  }
});
...
router.post("/others", async (req: Request, res: Response) => {
  try {
    ...
    if (!parsed.success) return res.status(400).json({ error: "Invalid", details: parsed.error.flatten() });
    const auditUserUuid = (req as any).user?.uuid || null;
...
router.patch("/others/:uuid", async (req: Request, res: Response) => {
  try {
