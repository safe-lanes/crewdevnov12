...
export const recWaitlistReport: ReportHandler<{ rank?: string; dateFrom?: string; dateTo?: string }> = {
  reportId: "rec-waitlist",
  title: "Waitlist",
  columns: [
    { key: "fileNo", label: "File No", type: "text", width: 110 },
    { key: "name", label: "Name", type: "text" },
    ...
    { key: "nationalityName", label: "Nationality", type: "text" },
    { key: "status", label: "Status", type: "status", width: 130 },
    { key: "lastUpdated", label: "Waitlisted Date", type: "date", width: 140 },
  ],
  ...
  async run(filters, ctx) {
    ...
    if (filters.rank) conds.push(eq(recruitmentCandidatesV2.rankAppliedFor, filters.rank));
    if (filters.dateFrom) conds.push(sql`${waitlistedDateExpr}::date >= ${filters.dateFrom}::date`);
    if (filters.dateTo) conds.push(sql`${waitlistedDateExpr}::date <= ${filters.dateTo}::date`);
    const where = and(...conds);
    const totalRes = await db
      .select({ c: sql<number>`count(*)` })
      .from(recruitmentCandidatesV2)
    ...
    const sortMap: Record<string, PgColumn | SQL> = {
      ...
      nationalityName: masterNationalities.nationality,
      status: recruitmentCandidatesV2.status,
      lastUpdated: waitlistedDateExpr,
    };
    const sortKey = ctx.sort?.key ?? "lastUpdated";
    const order = (ctx.sort?.direction === "desc" ? desc : asc)(sortMap[sortKey] ?? sortMap.lastUpdated);
    const rows = await db
...
const rejectedDateExpr = sql<string | null>`(
  SELECT NULLIF(TRIM(crd.recruitment_date), '')
  FROM cand_recruitment_decision crd
  WHERE crd.rec_can_uuid = recruitment_candidates_v2.rec_can_uuid
    AND crd.is_deleted = FALSE
...
export const recRejectedReport: ReportHandler<{ rank?: string; dateFrom?: string; dateTo?: string }> = {
  reportId: "rec-rejected",
  title: "Rejected",
  columns: [
    { key: "fileNo", label: "File No", type: "text", width: 110 },
    { key: "name", label: "Name", type: "text" },
    ...
    { key: "nationalityName", label: "Nationality", type: "text" },
    { key: "status", label: "Status", type: "status", width: 130 },
    { key: "lastUpdated", label: "Rejection Date", type: "date", width: 140 },
  ],
  filterSchema: z.object({
    rank: z.string().trim().min(1).optional(),
    dateFrom: dateFilter,
  ...
  async run(filters, ctx) {
    ...
    if (filters.rank) conds.push(eq(recruitmentCandidatesV2.rankAppliedFor, filters.rank));
    if (filters.dateFrom) conds.push(sql`${rejectedDateExpr}::date >= ${filters.dateFrom}::date`);
    if (filters.dateTo) conds.push(sql`${rejectedDateExpr}::date <= ${filters.dateTo}::date`);
    const where = and(...conds);
    const totalRes = await db
      .select({ c: sql<number>`count(*)` })
      .from(recruitmentCandidatesV2)
