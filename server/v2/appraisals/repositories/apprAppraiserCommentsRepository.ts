...
export class ApprAppraiserCommentsRepository {
  ...
  async syncForAppraisal(appraisalUuid: string, rows: any[], auditUserUuid?: string): Promise<void> {
    const db = getDb();
    await db.update(apprAppraiserCommentsV2)
      .set({ isDeleted: true, updatedAt: new Date() })
      .where(and(eq(apprAppraiserCommentsV2.appraisalUuid, appraisalUuid), eq(apprAppraiserCommentsV2.isDeleted, false)));
    if (rows.length > 0) {
      ...
      await db.insert(apprAppraiserCommentsV2).values(values);
    }
  }
}
