...
// UUID -> Vessel Name reverse mapping (from Master 014)
...
export function translateVesselNameToCode(vesselName: string): string {
  ...
  // If already a UUID format, return it (source of truth: Master 014)
  ...
  // Fail loudly if vessel cannot be translated - must be UUID format from Master 014
  throw new Error(
    `Cannot translate vessel name "${vesselName}" to canonical vessel code. ` +
    `Vessel must be in STATIC_VESSEL_MAPPING or already in UUID format (from Master 014). ` +
...
export interface IStorage {
  ...
  // Sea Service Entry Helpers
  upsertSeaServiceEntry(params: {
    crewId: string;
    planningId: number;
    vesselName: string;
    vesselCode: string;
  ...
  completeSeaServiceEntry(params: {
    crewId: string;
    planningId: number;
    signOffDate: string;
  }): Promise<void>;
  ...
  // Training Master
  getTrainingMasters(): Promise<TrainingMaster[]>;
  getTrainingMaster(id: number): Promise<TrainingMaster | undefined>;
  createTrainingMaster(training: InsertTrainingMaster): Promise<TrainingMaster>;
  updateTrainingMaster(id: number, training: Partial<UpdateTrainingMaster>): Promise<TrainingMaster | undefined>;
  deleteTrainingMaster(id: number): Promise<boolean>;
  reorderTrainingMasters(orders: Array<{ id: number; sortOrder: number }>): Promise<boolean>;
  // Company Training Groups
  getCompanyTrainingGroups(): Promise<CompanyTrainingGroup[]>;
  updateCompanyTrainingGroup(code: string, data: Partial<UpdateCompanyTrainingGroup>): Promise<CompanyTrainingGroup | undefined>;
  // Company Training
  ...
  // Promotion Reviews
  getPromotionReviews(): Promise<PromotionReview[]>;
  getPromotionReview(id: number): Promise<PromotionReview | undefined>;
  getPromotionReviewByCrewAndRank(crewMemberId: string, promotionToRank: string): Promise<PromotionReview | undefined>;
  getPromotionReviewsByCrewMember(crewMemberId: string): Promise<PromotionReview[]>;
...
export class MemStorage implements IStorage {
  ...
  async upsertSeaServiceEntry(params: {
    ...
    const existingIndex = currentSeaService.findIndex(record => 
      record.planningId === params.planningId && isActiveSeaService(record)
    );
  ...
  async completeSeaServiceEntry(params: {
    ...
    if (activeIndex >= 0) {
      ...
      // Calculate final period in months
      let finalPeriodMonths = '';
      if (from && to) {
        const months = calcPeriodMonths(from, to);
        ...
        for (let i = 0; i < planAssignments.length; i++) {
          ...
          if (filters?.archived ? isArchived : isPending) {
            ...
            if (assignment.vesselId) {
              // Use existing vesselId directly (UUID from Master 014)
              ...
              // Check if the vessel field already contains a vessel ID (UUID format)
              if (UUID_PATTERN.test(vesselValue)) {
                // It's already a vessel UUID, use it directly
                vesselIdToMatch = vesselValue;
              } else {
                // It's a vessel name, need to look it up in Master 014
                ...
                if (vessel && vessel.entryId) {
                  // Use the entryId (UUID from Master 014)
            ...
            if (vesselIdToMatch && assignment.rank) {
              ...
              // Note: crew.presentVessel stores vessel UUID from Master 014
...
export class PersistentFileStorage implements IStorage {
  ...
  private initializeDefaultData(): void {
    ...
    // Use the 6 vessels from Master 014 (UUID format)
    const vessels = [
