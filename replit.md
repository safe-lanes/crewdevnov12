...
## Recent Changes
- **2026-02-18**: Backend cleanup — removed ~190 dead /api/ route handlers from routes.ts (10,265→2,431 lines), cleaned storage.ts (8,325→5,301 lines) and database.ts (5,391→1,950 lines). Removed dead server modules (oilMajorRulesParser.ts, complianceEngine.ts). Total backend reduction: ~14,300 lines removed. Kept legacy routes still used by shared hooks: crew-members, masters, forms, available-ranks, company-ranks, training-master, company-trainings, pay-elements, contract-pay-elements, external sync. All v2 route registrations preserved.
