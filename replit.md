...
## Recent Changes
- **2026-02-23**: Implemented role-based access control (RBAC) for sidebar navigation. All module sidebars (Admin, Recruitment, Rotation, D&A, Rest Hours, Vessel, Crew Pool) now filter their sidebar items based on user permissions from PermissionsContext. Uses canView() with page-to-menu name mapping. Falls back to showing all items if no permissions loaded (graceful degradation). Fixed mobile nav to use filtered items. Backend API at `/api/v2/admin/access-control/my-permissions`.
