...
## Recent Changes
- **2026-02-23**: Fixed rotation plan multi-position rank assignment. Timeline now only shows positions that exist on the selected vessel (e.g., AB_1, AB_2, AB_3 — not AB_4 if vessel only has 3 AB slots). Added PositionSelectDialog popup that prompts users to pick a specific position (e.g., AB_1, AB_2, AB_3) when assigning a crewmember to a rank with multiple positions. Single-position ranks skip the popup. Editing existing assignments retains the position without showing the popup. Changes in NewPlanDialog_v2.tsx: added perVesselPositions map, PositionSelectDialog component, updated handleCrewSelect and autoSelectedRoleVariants logic. Backend fix: vesselRevisionsService now filters out redundant base rank rows (role=null) when a rank has multiple slots (slotCount > 1), so only actual positions are returned. Frontend fix: selectedVesselIdsForRanks now uses vessel UUIDs directly instead of translating through getVesselIds().
