...
## Recent Changes
- **2026-04-11**: Added frontend JWT token handling and auth redirect (Task #184). Created `client/src/lib/authToken.ts` for reading encrypted credentials from sessionStorage. Updated `tenantFetch.ts` and `queryClient.ts` to attach `Authorization: Bearer` header on all API requests. Global 401 handler redirects to parent login URL with redirect param. Startup auth check in `App.tsx` redirects if no credentials found (when encryption key is configured). Graceful degradation: skips auth when no encryption key is set (local dev).
- **2026-04-11**: Added JWT authentication middleware (Task #183). Backend validates JWT tokens from parent app (SAIL Audits) via `server/middleware/authMiddleware.ts`. Supports `Authorization: Bearer` header and `?sail=` query param for static files. Graceful degradation when `JWT_SECRET` not set in dev mode. Cross-tenant binding check prevents token from tenant A accessing tenant B. JWT `domain` field used as fallback for tenant resolution when `x-tenant-id` header missing. Env vars: `JWT_SECRET` (backend), `VITE_PARENT_LOGIN_URL` (frontend).
