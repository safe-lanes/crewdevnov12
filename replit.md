...
- **Crew Pool V2 Module**: Complete restructure following Recruitment V2 patterns with Repository + Service + Controller architecture.
  ...
  - **Master Data UUID Resolution** (`server/v2/crew-pool/services/masterDataResolver.ts`): Shared utility for resolving master data values to UUIDs. Uses strict UUID v4 regex validation. Pattern: Accept name OR UUID on WRITE operations, always store UUIDs, use JOINs on READ operations to return both UUID and resolved name. Applied to nationality, vesselType, country, and vessel fields across crew members, documents, visas, and profile services.
