...
- **Rank Ordering System**: All crew-displaying modules use rank-based sorting.
    - **useRankOrdering Hook (Jan 2026)**: Centralized hook (`client/src/hooks/useRankOrdering.ts`) that provides foolproof rank sorting. Queries vessel revision ranks directly from backend (via `/api/vessel-revisions/ranks/{vesselId}`) and builds a comprehensive rank order map from both `rank` and `role` fields. No reliance on hardcoded RANK_ALIASES - uses backend-provided sortOrder for all variants (AB_1, OS_2, Oiler_3, etc.). Exports `getSortOrder(rankName)` and `sortCrewByRank(crewArray)` helpers. Used by DrugAlcoholTestForm and can be reused across all modules needing consistent crew rank sorting.
