...
## Recent Changes
- **2026-02-27**: Flagged Violation Code EF detection logic as experimental. Added `CODE_EF_EXPERIMENTAL` toggle in `timelineCalculations.ts` (mirrors `WORK_ANCHORED_24H_WINDOW` pattern). Both detection paths (per-slot pipeline in `detectViolations` and per-day derivation in `RHRecordingForm.tsx`) are gated by this flag. Added "(Experimental)" label to EF description in all 6 violation display components. Current EF logic preserved as baseline for fine-tuning. Files: `timelineCalculations.ts`, `RHRecordingForm.tsx`, `ViolationsDetailDialog.tsx`, `ViolationsOverviewDialog.tsx`, `NCOverviewDialog.tsx`, `NCReportDialog.tsx`, `VesselReviewDialog.tsx`.
