...
- **Recruitment V2 Module**: Complete restructure from 1 table to 56 tables using Repository + Service + Controller pattern with complete isolation in v2 folder structure. Uses soft foreign keys (UUID as text) instead of database-level FK constraints. Four phases fully implemented:
  ...
  - Phase 2 (14 tables): Documents & Certificates (cand_travel_documents, cand_visas, cand_coc, cand_cop, cand_stcw_certificates, cand_flag_endorsements, cand_licenses, cand_medical_certificates, cand_training_certificates, cand_education, cand_sea_service_internal, cand_sea_service_external, cand_vaccinations, cand_document_attachments)
  - Phase 3 (29 tables): Screening B1-B8 (B1: General Screening - 4 tables, B2: Skills Assessment - 5 tables, B3: Interview Assessment - 4 tables, B4: Reference Checks - 4 tables, B5: Background Verification - 4 tables, B6: Psychological Assessment - 3 tables, B7: Medical Screening - 3 tables, B8: Final Evaluation - 2 tables)
  - Phase 4 (6 tables): Approvals & Decisions (hiring_decisions, offer_letters, employment_contracts, onboarding_tasks, decision_audit_trail, approval_workflows)
  - All API endpoints follow `/api/v2/recruitment/` pattern with recCanUuid in URL paths. Phase 4 endpoints use `/api/v2/recruitment/candidates/:recCanUuid/approvals/*` pattern
