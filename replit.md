...
- **Recruitment V2 Module**: Complete restructure from 1 table to 50 tables using Repository + Service + Controller pattern with complete isolation in v2 folder structure. Uses soft foreign keys (UUID as text) instead of database-level FK constraints. Three phases implemented:
  ...
  - All API endpoints follow `/api/v2/recruitment/` pattern with recCanUuid in URL paths
