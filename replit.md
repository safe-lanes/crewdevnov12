...
## Recent Changes
- **2026-02-26**: Changed "Hours of Rest in any 24 Hr Period" calculation for Violation Type 1 from rolling-window min-rest to work-anchored logic: `rest = 24 − maxWork` where maxWork is found across all valid 24h windows ending at-or-after the first work slot on the current day. Violation 1 detection is now derived directly from this per-day metric (< 10h = violation) instead of the old per-slot rolling-window pipeline. Red highlighting on hover shows the correct 24h window. Files: `timelineCalculations.ts` (`calculateRestIn24HWorkAnchored`), `RHRecordingForm.tsx`.
