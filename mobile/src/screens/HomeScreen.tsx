...
export default function HomeScreen() {
  ...
  return <View style={s.screen}><WelcomeBanner onPressBell={() => navigation.navigate("Notifications")} /><ScrollView refreshControl={<RefreshControl refreshing={loading} onRefresh={load} />} contentContainerStyle={s.content}><Text style={s.eyebrow}>SAIL CREW / ONBOARD COMPANION</Text><Text style={s.title}>Your crew records, ready when you are.</Text><Text style={s.subtitle}>A clear view of the details that keep every next step moving.</Text><StateView loading={loading} error={error} retry={load}>{<><View style={s.next}><View style={{ flex: 1 }}><Text style={s.nextLabel}>NEXT ACTION</Text><Text style={s.nextTitle}>{nextTitle}</Text><Text style={s.nextMeta}>{nextMeta}</Text></View><Text style={s.arrow}>›</Text></View><Text style={s.section}>Your workspace</Text><View style={s.grid}>{actions.map(([label, desc, icon, screen, collection]) => <Pressable key={label} testID={`dashboard-card-${label.toLowerCase().replace(/[^a-z]+/g, "-")}`} accessibilityRole="button" accessibilityLabel={`Open ${label}`} style={({ pressed }) => [s.action, pressed && s.pressed]} onPress={() => navigation.navigate(screen, collection ? { collection } : undefined)}><CrewIcon kind={icon} label={label} /><Text style={s.actionTitle}>{label}</Text><Text style={s.actionDesc}>{desc}</Text><Text style={s.actionArrow}>Open  ›</Text></Pressable>)}</View><View style={s.noticeHead}><Text style={s.section}>Recent notices</Text><Pressable accessibilityRole="button" accessibilityLabel="See all notices" onPress={() => navigation.navigate("Notices")}><Text style={s.seeAll}>See all</Text></Pressable></View>{notices.length ? notices.map(n => <Pressable accessibilityRole="button" accessibilityLabel={`Read notice ${n.title}`} key={n.noticeUuid} style={s.notice} onPress={() => navigation.navigate("Notices", { screen: "NoticeDetail", params: { noticeUuid: n.noticeUuid } })}><Text style={s.noticeTitle}>{n.title}</Text><Text style={s.noticeMeta}>Read notice  ›</Text></Pressable>) : <Text style={s.muted}>No recent notices.</Text>}</>}</StateView></ScrollView></View>;
}
const s = StyleSheet.create({ screen: { flex: 1, backgroundColor: palette.mist }, content: { padding: 20, paddingBottom: 40 }, eyebrow: { color: palette.teal, fontWeight: "800", fontSize: 11, letterSpacing: 1.2, marginTop: 5 }, title: { fontSize: 29, lineHeight: 34, fontWeight: "800", color: palette.navy, marginTop: 8 }, subtitle: { color: palette.muted, fontSize: 15, lineHeight: 21, marginTop: 8 }, next: { backgroundColor: palette.navy, borderRadius: 16, padding: 18, marginTop: 22, flexDirection: "row", justifyContent: "space-between", alignItems: "center" }, nextLabel: { color: "#7FCDD0", fontWeight: "800", fontSize: 11, letterSpacing: 1 }, nextTitle: { color: "#FFF", fontWeight: "800", fontSize: 16, marginTop: 7 }, nextMeta: { color: "#C4D8E0", marginTop: 4 }, arrow: { color: "#7FCDD0", fontSize: 32 }, section: { color: palette.navy, fontWeight: "800", fontSize: 18, marginTop: 24, marginBottom: 10 }, grid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between" }, action: { width: "48.2%", minHeight: 165, backgroundColor: "#FFF", borderRadius: 15, borderWidth: 1, borderColor: "#D8E6E9", padding: 15, marginBottom: 10 }, icon: { width: 38, height: 34, alignItems: "center", justifyContent: "center", marginBottom: 10, position: "relative" }, iconHead: { width: 11, height: 11, borderRadius: 6, borderWidth: 2, borderColor: palette.teal, position: "absolute", top: 2 }, iconShoulders: { width: 28, height: 14, borderTopLeftRadius: 14, borderTopRightRadius: 14, borderWidth: 2, borderBottomWidth: 0, borderColor: palette.teal, position: "absolute", bottom: 2 }, iconPage: { width: 24, height: 30, borderRadius: 3, borderWidth: 2, borderColor: palette.teal }, iconLine: { width: 12, height: 2, backgroundColor: palette.teal, position: "absolute" }, iconAward: { width: 20, height: 20, borderRadius: 10, borderWidth: 2, borderColor: palette.teal, position: "absolute", top: 1 }, iconRibbon: { width: 7, height: 15, borderLeftWidth: 2, borderColor: palette.teal, position: "absolute", bottom: 0, transform: [{ rotate: "12deg" }] }, iconMast: { width: 2, height: 22, backgroundColor: palette.teal, position: "absolute", top: 1, left: 18 }, iconSail: { width: 0, height: 0, borderTopWidth: 9, borderBottomWidth: 9, borderLeftWidth: 13, borderTopColor: "transparent", borderBottomColor: "transparent", borderLeftColor: palette.teal, position: "absolute", top: 3, left: 20 }, iconHull: { width: 32, height: 9, borderBottomLeftRadius: 15, borderBottomRightRadius: 15, borderWidth: 2, borderTopWidth: 0, borderColor: palette.teal, position: "absolute", bottom: 1 }, iconCrossVertical: { width: 9, height: 30, borderRadius: 2, backgroundColor: palette.teal, position: "absolute" }, iconCrossHorizontal: { width: 30, height: 9, borderRadius: 2, backgroundColor: palette.teal, position: "absolute" }, iconBubble: { width: 32, height: 26, borderRadius: 6, borderWidth: 2, borderColor: palette.teal }, actionTitle: { color: palette.navy, fontWeight: "800", fontSize: 16, lineHeight: 20 }, actionDesc: { color: palette.muted, fontSize: 12, marginTop: 7, lineHeight: 17 }, actionArrow: { color: palette.teal, fontWeight: "800", fontSize: 12, marginTop: "auto" }, pressed: { opacity: .7, transform: [{ scale: .985 }] }, noticeHead: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" }, seeAll: { color: palette.teal, fontWeight: "800" }, notice: { backgroundColor: "#FFF", borderRadius: 12, padding: 15, borderWidth: 1, borderColor: "#D8E6E9", marginBottom: 8 }, noticeTitle: { color: palette.ink, fontWeight: "700" }, noticeMeta: { color: palette.muted, fontSize: 12, marginTop: 7 }, muted: { color: palette.muted } });