...
-- Phase 2 Additions:
--   - company_processing table (9 columns, 3 indexes)
--   - promotion_forms table (14 columns, 3 indexes, 2 foreign keys)
--   - 8 rank_groups data records for appraisal form mapping
--
-- Tables: 34 (32 original + 2 new)
-- Indexes: +6 (Phase 2)
-- Foreign Keys: +2 (Phase 2)
-- ============================================
...
-- ============================================
DROP SEQUENCE IF EXISTS public.promotion_forms_id_seq;
DROP TABLE IF EXISTS public.promotion_forms;
DROP SEQUENCE IF EXISTS public.company_processing_id_seq;
DROP TABLE IF EXISTS public.company_processing;
...
-- Sequence for company_processing
CREATE SEQUENCE IF NOT EXISTS public.company_processing_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS public.company_processing (
    id integer NOT NULL DEFAULT nextval('company_processing_id_seq'::regclass),
...
ALTER SEQUENCE public.company_processing_id_seq OWNED BY public.company_processing.id;

-- Sequence for promotion_forms
CREATE SEQUENCE IF NOT EXISTS public.promotion_forms_id_seq
    START WITH 1
    INCREMENT BY 1
    NO MINVALUE
    NO MAXVALUE
...
CREATE TABLE IF NOT EXISTS public.promotion_forms (
    id integer NOT NULL DEFAULT nextval('promotion_forms_id_seq'::regclass),
...
ALTER SEQUENCE public.promotion_forms_id_seq OWNED BY public.promotion_forms.id;
...
-- ============================================
-- Appraisal form mappings for all 19 maritime ranks

INSERT INTO public.rank_groups (form_id, name, ranks) VALUES
...
-- Primary keys for Phase 2 tables
ALTER TABLE ONLY public.company_processing
    ADD CONSTRAINT company_processing_pkey PRIMARY KEY (id);

ALTER TABLE ONLY public.promotion_forms
    ADD CONSTRAINT promotion_forms_pkey PRIMARY KEY (id);

-- Indexes for company_processing
CREATE INDEX idx_company_processing_candidate_id ON public.company_processing USING btree (candidate_id);
CREATE INDEX idx_company_processing_process_type ON public.company_processing USING btree (process_type);
CREATE INDEX idx_company_processing_status ON public.company_processing USING btree (status);

-- Indexes for promotion_forms
CREATE INDEX idx_promotion_forms_crew_member_id ON public.promotion_forms USING btree (crew_member_id);
CREATE INDEX idx_promotion_forms_status ON public.promotion_forms USING btree (status);
CREATE INDEX idx_promotion_forms_appraisal_result_id ON public.promotion_forms USING btree (appraisal_result_id);

-- Foreign keys for promotion_forms
ALTER TABLE ONLY public.promotion_forms
    ADD CONSTRAINT promotion_forms_crew_member_id_crew_members_id_fk 
    FOREIGN KEY (crew_member_id) REFERENCES public.crew_members(id) ON DELETE CASCADE;

ALTER TABLE ONLY public.promotion_forms
    ADD CONSTRAINT promotion_forms_appraisal_result_id_appraisal_results_id_fk 
    FOREIGN KEY (appraisal_result_id) REFERENCES public.appraisal_results(id) ON DELETE SET NULL;

--
-- PostgreSQL database dump complete
--
-- Phase 2 Complete - Version 2.0
--
