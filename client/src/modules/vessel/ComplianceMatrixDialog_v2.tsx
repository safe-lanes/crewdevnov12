...
interface SimulatedCrewMemberV2 {
    rank: string;
    crewMemberId: string;
    crewName?: string;
    joiningDate?: string;
...
interface ComplianceMatrixDialogV2Props {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    vesselId?: string;
    simulatedCrew?: SimulatedCrewMemberV2[];
}
...
export function ComplianceMatrixDialog_v2({
    open,
    onOpenChange,
    vesselId,
    simulatedCrew
}: ComplianceMatrixDialogV2Props) {
    ...
    const { data: complianceData, isLoading: isLoadingCompliance } = useQuery<{ vesselId: string; results: ComplianceCheckResult[]; message?: string; simulated?: boolean }>({
        queryKey: isSimulatedMode 
            ? [`/api/v2/compliance/matrix/${vesselId}/simulated`, JSON.stringify(simulatedCrew)]
                                                    ...
                                                    <TableBody>
                                                        {isEnglishProficiency && groupedProficiency ? (
                                                                                            ...
                                                                                            {group.individualResults.map((indiv, indivIndex) => {
                                                                                                ...
                                                                                                return (
                                                                                                    <TableRow key={indivIndex} className="border-b border-gray-100">
                                                                                                        <TableCell className="text-xs text-gray-600 py-2">{indiv.rankPair}</TableCell>
                                                                                                        <TableCell className="text-xs text-gray-600 py-2 text-center">{indivRequired}</TableCell>
                                                                                                        <TableCell className="text-xs text-gray-600 py-2 text-center">{indivActual}</TableCell>
                                                                                                    </TableRow>
                                                                                                );
                                                                                            })}
                                                                                        </TableBody>
                                                            ...
                                                            requirements.map((req, reqIndex) => {
                                                                const { required, actual } = formatRequirementValue(req);
                                                                return (
                                                                    <TableRow 
                                                                        key={reqIndex} 
                                                                        className="border-b border-gray-100"
                                                                    ...
                                                                    >
                                                                        <TableCell className="text-xs text-gray-600">
                                                                            {req.label}
                                                                            {req.rankPair && (
                                                                                <span className="text-gray-400 ml-2">({req.rankPair})</span>
                                                                            )}
                                                                        </TableCell>
                                                                        <TableCell className="text-xs text-gray-600 text-center">
                                                                            {required}
