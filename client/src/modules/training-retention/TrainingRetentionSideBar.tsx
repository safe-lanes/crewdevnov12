...
interface TrainingRetentionSideBarProps {
    selectedPage: string;
    setSelectedPage: (page: string) => void;
    allowedPages: string[];
}

const trainingRetentionSideBarList: { name: string; icon: React.ReactNode; page: string }[] = [
    {
        name: "Training",
...
export default function TrainingRetentionSideBar({ selectedPage, setSelectedPage, allowedPages }: TrainingRetentionSideBarProps) {
    ...
    return (
        <TooltipProvider>
            ...
            >
                {
                    trainingRetentionSideBarList.filter(item => allowedPages.includes(item.page)).map(item => (
                            ...
                            <TooltipTrigger asChild>
                                <div
                                    className={`w-full flex flex-col items-center justify-center cursor-pointer flex-shrink-0 transition-all duration-200 ${
                                        selectedPage === item.page ? "bg-[#52baf3]" : "bg-[#16569e] hover:bg-[#1e5fa8]"
                                    }`}
