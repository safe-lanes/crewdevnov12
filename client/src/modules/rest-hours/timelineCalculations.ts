 ...
 * work-anchored logic:
 *
 *   1. Find the first work slot on Day N (anchorDayIndices).
 ...
 *   3. Additionally, windows that fall entirely within Day N but contain the
 *      highest work hours are also valid candidates.
 *      (Note 2: if the day alone yields a higher work figure, use that.)
 *   4. maxWork = the highest work hours found across all valid candidate windows.
 *   5. Return { rest: 24 − maxWork, worstWindowEndSlot } so callers can also
 *      derive the violation highlight range.
 *
 * Fallback: if anchorDayIndices is empty or has no work at all, falls back
 * to calculateMinRestInAny24HourPeriod (original logic).
 *
 * @param fallbackIndices  - Array positions used for the fallback calculation
 * @param anchorDayIndices - Array positions in fullTimeline for the anchor day
 * @param fullTimeline     - The complete timeline slot array
 * @param cumulativeWork   - Prefix sum array for work hours
 * @param cumulativeRest   - Prefix sum array for rest hours
 * @returns { rest, worstWindowEndSlot }
 */
export function calculateRestIn24HWorkAnchored(
  fallbackIndices: number[],
  anchorDayIndices: number[],
  fullTimeline: TimelineSlot[],
  cumulativeWork: number[],
  cumulativeRest: number[]
): { rest: number; worstWindowEndSlot: number | null } {
  const WINDOW_SIZE_24H = 48;

  if (anchorDayIndices.length === 0) {
    return {
      rest: calculateMinRestInAny24HourPeriod(fallbackIndices, cumulativeRest),
      worstWindowEndSlot: null,
    };
  }

  let firstWorkIdx = -1;
  for (const idx of anchorDayIndices) {
  ...
  for (const endIdx of anchorDayIndices) {
    if (endIdx < firstWorkIdx) continue;
    if (endIdx < WINDOW_SIZE_24H - 1) continue;
  ...
  return { rest: 24 - maxWork, worstWindowEndSlot: worstEndSlot };
}
...
export const VIOLATION_SORT_ORDER: Record<string, number> = {
  'A': 1, 'B': 2, 'C': 3, 'D': 4, 'EF': 5, 'G': 6, 'H': 7, 'I': 8,
};
