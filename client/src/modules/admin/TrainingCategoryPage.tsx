...
import {
  useTrainingCategoriesV2,
  TRAINING_CATEGORIES_KEY,
  type TrainingCategoryV2,
...
export default function TrainingCategoryPage() {
  ...
  const { data: categories = [], isLoading } = useTrainingCategoriesV2();
  const [dialog, setDialog] = useState<DialogState>(null);
  const [label, setLabel] = useState("");
  const [selectedModules, setSelectedModules] = useState<string[]>([]);
  const [deleteTarget, setDeleteTarget] = useState<TrainingCategoryV2 | null>(null);

  const invalidate = () => queryClient.invalidateQueries({ queryKey: [TRAINING_CATEGORIES_KEY] });

  const createMutation = useMutation({
    mutationFn: async (data: { label: string; modules: string[] }) => {
      const res = await apiRequest("POST", TRAINING_CATEGORIES_KEY, {
        ...data,
    ...
    onSuccess: () => {
      invalidate();
      setDialog(null);
      toast({ title: "Training category created" });
    },
    onError: (e: Error) =>
  ...
  const groupUpdateMutation = useMutation({
    mutationFn: async (data: { originalLabel: string; label: string; modules: string[] }) => {
      const res = await apiRequest("PUT", `${TRAINING_CATEGORIES_KEY}/group`, {
        ...data,
        auditUserUuid: getCrewUserId(),
      });
      return res.json();
  ...
  const toggleActiveMutation = useMutation({
    mutationFn: async ({ mtcUuid, isActive }: { mtcUuid: string; isActive: boolean }) => {
      const res = await apiRequest("PUT", `${TRAINING_CATEGORIES_KEY}/${mtcUuid}`, {
        isActive,
        auditUserUuid: getCrewUserId(),
      });
      return res.json();
  ...
  const deleteMutation = useMutation({
    mutationFn: async (mtcUuid: string) => {
      const res = await apiRequest(
        "DELETE",
        `${TRAINING_CATEGORIES_KEY}/${mtcUuid}?auditUserUuid=${encodeURIComponent(getCrewUserId() || "")}`,
      );
      return res.json();
    },
    onSuccess: () => {
      ...
      toast({ title: "Training category deleted" });
    },
    onError: (e: Error) =>
  ...
  const openEdit = (row: TrainingCategoryV2) => {
    const modules = categories.filter((s) => s.label === row.label).map((s) => s.module);
    setLabel(row.label);
  ...
  const handleSave = () => {
    ...
    if (!trimmed) {
      toast({ title: "Category label is required", variant: "destructive" });
      return;
    }
    ...
    <div>
          ...
          <Table>
            <TableHeader className="sticky top-0 z-10">
              <TableRow className="bg-[#52baf3] hover:bg-[#52baf3]">
                <TableHead className="text-xs text-white">Category</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={4} className="text-center text-xs py-6" data-testid="text-no-categories">
                    No training categories found
                  </TableCell>
                </TableRow>
              ) : (
                filteredRows.map((row) => (
                  <TableRow key={row.mtcUuid} data-testid={`row-training-category-${row.mtcUuid}`}>
                    <TableCell className="text-xs" data-testid={`text-category-label-${row.mtcUuid}`}>
                      {row.label}
                    </TableCell>
                    ...
                    <TableCell>
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          toggleActiveMutation.mutate({ mtcUuid: row.mtcUuid, isActive: checked === true })
                        }
                        data-testid={`checkbox-category-active-${row.mtcUuid}`}
                      />
                    </TableCell>
                    <TableCell>
                      <div className="flex items-center gap-1">
                        <Button
                          ...
                          data-testid={`button-edit-category-${row.mtcUuid}`}
                        >
                        ...
                        <Button
                          ...
                          data-testid={`button-delete-category-${row.mtcUuid}`}
                        >
                        ...
                        <Button
                          ...
                          className="h-6 w-6"
                          onClick={openCreate}
                          data-testid={`button-add-category-${row.mtcUuid}`}
                        >
      ...
      <Dialog open={dialog !== null} onOpenChange={(open) => !open && setDialog(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>
              {dialog?.mode === "edit" ? "Edit Training Category" : "Add Training Category"}
            </DialogTitle>
            <DialogDescription>
              {dialog?.mode === "edit"
                ? "Rename the category or change its module assignments. Deselecting a module removes the category from that module."
                : "Selecting multiple modules creates the category independently in each module."}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            <Button
              ...
              data-testid="button-cancel-category-dialog"
            >
              Cancel
            </Button>
            <Button
      ...
      <AlertDialog open={deleteTarget !== null} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Training Category</AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              onClick={() => deleteTarget && deleteMutation.mutate(deleteTarget.mtcUuid)}
