...
export default function AccessControlPage() {
  ...
  const handleSelectAll = useCallback(
    (menuMuid: string, checked: boolean, restricted = false, noDelete = false) => {
      setLocalPermissions((prev) => ({
        ...prev,
        [menuMuid]: {
          canview: checked,
          ...
          candelete: (restricted || noDelete) ? false : checked,
        },
      }));
      setIsDirty(true);
    },
  ...
  const renderMenuRow = (item: MenuTreeItem, depth: number = 0) => {
    ...
    const someSelected = !allSelected && (perms.canview || perms.cancreate || (!isLockMenu && (perms.canedit || (!isCpFormMenu && perms.candelete))));
        ...
        >
          ...
          <div className="flex items-center justify-center py-2.5">
            <Checkbox
              ...
              onCheckedChange={() =>
                handleSelectAll(item.muid, !allSelected, isLockMenu, isCpFormMenu)
              }
          ...
          {PERMISSION_KEYS.map((key) => {
            const isDisabledKey = (isLockMenu && LOCK_ACTION_DISABLED_KEYS.has(key)) || (isCpFormMenu && CP_FORM_DISABLED_KEYS.has(key));
            return (
              <div
                key={key}
