-- Migration 0162: Register the Accounts Vessel Portage page in RBAC.
--
-- Adds the vessel-side submission package workspace as an RBAC-gated menu
-- under the top-level 'Account' menu:
--   - Account Vessel Portage  /accounts/payroll/vessel-portage
--
-- Role permissions are seeded by copying each role's grant on the top-level
-- 'Account' menu (same pattern as migrations 0155/0158/0160). Roles with no
...
--    Sort order continues after the 0160 page (8).
-- =========================================================================
...
FROM (VALUES
  ('Account Vessel Portage', 'Vessel Portage', '/accounts/payroll/vessel-portage', 9)
...
WHERE m.name IN ('Account Vessel Portage')
AND NOT EXISTS (
